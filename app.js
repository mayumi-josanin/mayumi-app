...
async function initApp() {
  ...
  // **待たずに、その場で数を出す。**（院長の希望 2026-09-20「開いたらすぐに正しい画面を」）
  //
  // 端末には前回の数が残っている。ふだんはそれが正しいので、すぐ出して構わない。
  // 食い違うのは**管理画面で数を訂正した直後だけ**で、そのときは下の取り直しで
  // 静かに書き換わる。めったに無いことのために、毎回みんなを待たせない。
  //
  // 設計図（index.html）の見本の数字は空にしてあるので、**間違った数が焼き付いて
  // 見えることはない**。出るのは端末に入っている本物の数だけ。
  updateStampUI();
  起動の伏せを解く_();

  // 取り直しは後ろで走らせる。違っていたら静かに書き換わる。
  loadStampRewards().catch(function () { /* 取れなくても、いま出ている数のままでよい */ });
  checkUrlParams();
