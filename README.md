...
このリポジトリは公開されています。**認証にかかわる値は、コードにも README にも書かず、必ずスクリプト プロパティに設定してください。**

| キー | 内容 | 未設定だとどうなるか |
|---|---|---|
| `TOKEN_SECRET` | 管理トークンの署名鍵。32文字以上のランダム文字列 | **危険。** `gas/Code.gs` の既定値が使われる。既定値は公開されているため、**ログインを通らずに管理トークンを偽造できる** |
| `ADMIN_USERNAME` | 管理アプリのログインID | コード側の初期値が使われる |
| `ADMIN_PASSWORD` | 管理アプリのパスワード。英数字記号を混ぜて16文字以上 | コード側の初期値が使われる |
| `SPREADSHEET_ID` | 保存先スプレッドシートのID | `gas/Code.gs` の既定値を使う |

**`TOKEN_SECRET` は最優先です。** ここが未設定だと、`ADMIN_PASSWORD` をどれだけ強くしても迂回されます。

新しい `TOKEN_SECRET` を設定すると、発行済みの管理トークンは無効になります。管理アプリからログインし直してください。

管理アプリの「認証設定」画面からログインIDとパスワードを変更できます。パスワード欄を空欄で保存した場合は現在のパスワードを維持します。
...
お客様情報はフルネームのみを使用します。メールアドレス入力はありません。
...
お客様アプリのホーム画面ではアンケート一覧だけを表示し、各ボタンを押すと該当アンケートの質問画面へ移動します。
