...
def カレンダー画像(request, 名前):
    return _配る("calendar", 名前)


def 開発画像(request, 名前):
    """`/media/dev/<名前>` を配る（目安箱のスクリーンショット）。"""
    return _配る("dev", 名前)


def ビジリス画像(request, 名前):
    """`/media/bijiris/<名前>` を配る（ビジリスの回答の写真。Drive から移したもの）。

    お客様のお体の写真。ビジリスの GAS は 2026-08-25 から公開をやめ、札を確かめて渡している
    （`getPhotoData_`）。ここも同じにしたいが、他の画像と同じく「名前を知っていれば見える」で始める
    （名前は乱数）。**合鍵を求める形に変えるのはこの1か所で済む。**
    """
    return _配る("bijiris", 名前)
