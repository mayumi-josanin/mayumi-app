...
def _日付(v):
    ...
    管理アプリの「日時」欄は datetime-local（`2026-09-14T10:30`）で送ってくる。
    時刻が付いたままだと日付として読めず、**今日の日付に化けていた。**
    投稿日は日付だけ持つ表なので、時刻は落として日付だけ読む。
    """
