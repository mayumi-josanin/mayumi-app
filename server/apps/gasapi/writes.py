...
def _宛先で振り分ける(お知らせの関数, メニューの関数, 商品の関数=None, カレンダーの関数=None):
    """`sheet` を見て、その表の窓口へ渡す。
...
def _カレンダー(名):
    def 呼ぶ(d):
        ...
        return getattr(admin_calendar, 名)(d)
    return 呼ぶ


_カレンダー足す = _カレンダー("足す")
_カレンダー書き換える = _カレンダー("書き換える")

_一覧掲載を変える = _宛先で振り分ける(_お知らせ一覧掲載, _メニュー("一覧掲載を変える"),
                                    _商品("一覧掲載を変える"), _カレンダー("一覧掲載を変える"))
_一覧から外す = _宛先で振り分ける(_お知らせ一覧から外す, _メニュー("一覧から外す"),
                                _商品("一覧から外す"), _カレンダー("一覧から外す"))
_公開を変える = _宛先で振り分ける(_お知らせ公開, None, _商品("公開を変える"), _カレンダー("公開を変える"))
def _通知(名):
    def 呼ぶ(d):
        from . import admin_push

        return getattr(admin_push, 名)(d)
    return 呼ぶ


def _PUSHも(元, 通知の関数):
    """`sheet: 'PUSH'`（通知の履歴）が来たら通知の表へ。それ以外は元の振り分けへ。"""
    def 呼ぶ(d):
        if str((d or {}).get("sheet") or "").strip().upper() == "PUSH":
            return 通知の関数(d)
        return 元(d)
    return 呼ぶ


_消す = _PUSHも(_宛先で振り分ける(_お知らせ消す, None, _商品("消す"), _カレンダー("消す")),
              _通知("消す"))
_まとめて消す = _PUSHも(_宛先で振り分ける(_お知らせまとめて消す, None, _商品("まとめて消す"),
                                     _カレンダー("まとめて消す")), _通知("まとめて消す"))
...
書けること = {
    ...
    "deleteRow": _消す,
    "deleteRows": _まとめて消す,
    # プッシュ通知の表（2026-09-14）。GAS の broadcastPush / sendAutoPush（mode: auto）/
    # 5分ごとの予約送信トリガーが転送してくる。送るのもここ（apps/content/onesignal.py）。
    "broadcastPush": _通知("送る"),
    "processScheduledPushQueue": _通知("予約分を送る"),
