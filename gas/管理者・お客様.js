...
const CONFIG = {
  ...
  CHANNEL_ACCESS_TOKEN: '__SET_IN_APPS_SCRIPT__',
  USER_ID: '__SET_IN_APPS_SCRIPT__',

  GMAIL_TO: 'josanin.mayumi@gmail.com',
  SHEET_NAME: 'まゆみ助産院_管理',
  ADMIN_NAME: 'まゆみ助産院',
  // OneSignal設定
  ONESIGNAL_APP_ID: '__SET_IN_APPS_SCRIPT__',
  ONESIGNAL_REST_API_KEY: '__SET_IN_APPS_SCRIPT__',
...
const NOTICE_DELETED_AT_HEADER = 'お知らせ一覧削除日時';
  ...
  try {
    ...
    switch (type) {
      ...
      case 'updateNoticeVisibility':
        ...
        break;
      case 'deleteNoticeListing':
        result = handleDeleteNoticeListing(data);
        break;
      case 'addBlog':
...
function parseStoredImageUrls_(value) {
  if (Array.isArray(value)) {
    return value.reduce(function (acc, item) {
      return acc.concat(parseStoredImageUrls_(item));
    }, []);
  }
  const raw = String(value || '').trim();
  if (!raw) return [];
  if (raw.charAt(0) === '[') {
    try {
      ...
      if (Array.isArray(parsed)) return parseStoredImageUrls_(parsed);
    } catch (err) {
      Logger.log('parseStoredImageUrls_ parse error: ' + err.toString());
    }
  }
  ...
  return candidates.filter(function (item, index) {
    return /^(https?:\/\/|data:image\/)/i.test(item) && candidates.indexOf(item) === index;
  });
}

function serializeStoredImageUrls_(value) {
  const urls = parseStoredImageUrls_(value);
  if (!urls.length) return '';
  return JSON.stringify(urls);
}

function getBlogImageUrlsFromRow_(row, imageCol, body) {
  const storedImageUrls = imageCol ? parseStoredImageUrls_(row[imageCol - 1]) : [];
  if (storedImageUrls.length) return storedImageUrls;
...
function getBlogImageUrlFromRow_(row, imageCol, body) {
  const urls = getBlogImageUrlsFromRow_(row, imageCol, body);
...
function getBlogNews() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const imageUrls = getBlogImageUrlsFromRow_(row, imageCol, body);
...
function getProducts() {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (isNoticeListingDeleted_(row, deletedAtCol)) continue;
    if (isSoftDeletedByColumns_(row, deleteCols.statusCol, deleteCols.deletedAtCol)) continue;
    if (!isPublishAtAvailable_(row[publishAtCol - 1])) continue;
    ...
    products.push({
      ...
      name: String(row[1]),
      price: Number(row[2]),
      icon: imageUrls.length ? imageUrls[0] : String(row[3] || '🌿'),
      imageUrls: imageUrls,
      bg: String(row[4] || 'c1'),
      description: String(row[6] || ''),
      descriptionImage: descriptionImageUrls.length ? descriptionImageUrls[0] : String(row[7] || ''),
...
function ensureNoticeDeletedAtColumn_(sheet) {
  if (!sheet) return 0;
  return ensureNamedColumn_(sheet, NOTICE_DELETED_AT_HEADER, 160);
}

function isNoticeListingDeleted_(row, deletedAtCol) {
  if (!deletedAtCol) return false;
  return String((row && row[deletedAtCol - 1]) || '').trim() !== '';
}
...
function shouldSendManagedContentPush_(data) {
  const value = data && data.sendPushNotification;
  return value === true
    || value === 'true'
    || value === '1'
    || value === 1
...
function handleAddBlog(data) {
  try {
    ...
    if (imageCol) {
      sheet.getRange(rowIdx, imageCol).setValue(
        serializeStoredImageUrls_(data.imageUrls && data.imageUrls.length ? data.imageUrls : (data.image || ''))
      );
    }
...
function handleUpdateBlog(data) {
  try {
    ...
    if (imageCol && (data.image !== undefined || data.imageUrls !== undefined)) {
      sheet.getRange(rowIdx, imageCol).setValue(
        serializeStoredImageUrls_(data.imageUrls && data.imageUrls.length ? data.imageUrls : (data.image || ''))
      );
    }
    ...
    if (shouldSendManagedContentPush_(data) && String(effectiveStatus || '公開') !== '非公開' && isPublishAtAvailable_(effectivePublishAt)) {
...
function handleUpdateNoticeVisibility(data) {
  try {
    ...
    if (deletedAtCol > 0) {
      sheet.getRange(rowIdx, deletedAtCol).setValue('');
    }
    bumpDataCacheVersion_();
    return { status: 'ok' };
  } catch (err) {
...
function handleDeleteNoticeListing(data) {
  try {
    ...
    const noticeCol = ensureNoticeVisibilityColumn_(sheet, sourceStatusCol, '公開');
    const deletedAtCol = ensureNoticeDeletedAtColumn_(sheet);
    sheet.getRange(rowIdx, noticeCol).setValue('非公開');
    sheet.getRange(rowIdx, deletedAtCol).setValue(formatDateTime_(new Date()));
    bumpDataCacheVersion_();
    return { status: 'ok' };
  } catch (err) {
    Logger.log('handleDeleteNoticeListing error: ' + err.toString());
    return { status: 'error', message: err.toString() };
  }
}
...
function handleAddProduct(data) {
  try {
    ...
    prodSheet.appendRow([
      ...
      data.name,
      Number(data.price),
      serializeStoredImageUrls_(data.imageUrls && data.imageUrls.length ? data.imageUrls : (data.icon || '')),
      data.bg || 'c1',
      data.status || '公開',
      data.description || '',
...
function handleUpdateProduct(data) {
  try {
    ...
    if (data.icon !== undefined || data.imageUrls !== undefined) {
...
function getCalendarEvents() {
  try {
    ...
    const events = values.map(function (row, index) {
      if (row[4] !== '公開' ||
        isNoticeListingDeleted_(row, deletedAtCol) ||
      ...
      return {
        ...
        image: parseStoredImageUrls_(row[5]).length ? parseStoredImageUrls_(row[5])[0] : String(row[5] || ''),
...
function getBlogNews() {
  try {
    ...
    const categoryTypeMap = {};
    ((categoriesRes && categoriesRes.categories) || []).forEach(function (item) {
      const name = String(item && item.name || '').trim();
      if (!name) return;
      categoryTypeMap[name] = item.type === 'お知らせ' ? 'お知らせ' : 'ブログ';
    });
    ...
    const news = values.map(function (row, index) {
      if (isNoticeListingDeleted_(row, deletedAtCol) ||
        isSoftDeletedByColumns_(row, deleteCols.statusCol, deleteCols.deletedAtCol) ||
        normalizePublishVisibilityStatus_(row[noticeCol - 1] || row[5] || '公開') === '非公開' ||
        !isPublishAtAvailable_(row[publishAtCol - 1])) {
        return null;
      }
      const body = String(row[4] || '');
...
function getDefaultSupportFaqRows_(updatedAt) {
  ...
  return [
    ...
    ['公開', 'スタンプ特典', '特典はどこで確認できますか？', '特典,どこ,確認,プレゼント,ガチャ', '特典はマイページの「🎁 スタンプ・特典履歴」で確認できます。未使用のガチャ特典と受取期限を確認でき、使用した特典は履歴から表示されません。', 102, now],
  ...
  try {
    ...
    const menus = data
      .map(function (row, index) {
        ...
        const imageUrls = parseStoredImageUrls_(row[2]);
        return {
          rowIdx: index + 2,
          date: formatMaybeDateTime_(row[0]),
          name: row[1],
          imageUrl: imageUrls.length ? imageUrls[0] : row[2],
          imageUrls: imageUrls,
          description: row[3],
...
function handleAddMenu(data) {
  try {
    ...
    rowData[0] = today;
    rowData[1] = data.name || '';
    rowData[2] = serializeStoredImageUrls_(data.imageUrls && data.imageUrls.length ? data.imageUrls : (data.imageUrl || ''));
    ...
    if (shouldSendManagedContentPush_(data) && String(data.publishStatus || '非公開') === '公開' && isPublishAtAvailable_(data.publishAt)) {
  ...
  try {
    ...
    if (data.imageUrl !== undefined || data.imageUrls !== undefined) {
      sheet.getRange(rowIdx, 3).setValue(
        serializeStoredImageUrls_(data.imageUrls && data.imageUrls.length ? data.imageUrls : (data.imageUrl || ''))
      );
    }
    ...
    const effectivePublishAt = publishAtCol ? sheet.getRange(rowIdx, publishAtCol).getValue() : '';
    if (shouldSendManagedContentPush_(data) && String(effectiveStatus || '非公開') === '公開' && isPublishAtAvailable_(effectivePublishAt)) {
      sendAutoPush('🍴 ' + (data.name || 'ホーム更新'), 'ホームのメニュー一覧が更新されました', {
        targetPage: 'menu-list'
      });
    }
