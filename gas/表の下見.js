...
function 表の下見() {
  ...
  表下見_対象.forEach(function (名) {
    ...
    // 大きい表は先頭だけ見る。**そのことを必ず書く。**
    // 「◯件」とだけ出すと、シート全体の数だと読めてしまう。
    // 操作履歴は9,500行あり、200行だけ見た結果を全体と取り違えると、
    // 「この列は200件しか入っていない」と誤って判断する。
    var 見る行数 = Math.min(最終行, 200);
    var 全部見たか = 見る行数 >= 最終行;

    var v = sh.getRange(1, 1, 見る行数, 最終列).getValues();
    var 見出し = v[0];
    var 母数 = v.length - 1;
    ...
    if (全部見たか) {
      ...
      Logger.log('  列と、埋まっている件数'
        + '（**先頭' + 母数 + '行だけを見た結果です。全' + (最終行 - 1) + '行ではありません**）:');
    }
...
function 表の内訳() {
  ...
  表下見_内訳.forEach(function (指定) {
    ...
    // 種類が多いときは、件数の多い順に並べたほうが読める。
    // 出てきた順のままだと、大事な多数派が下のほうに埋もれる。
    if (指定.多い順) {
      並び.sort(function (a, b) { return 集計[b].件数 - 集計[a].件数; });
    }
    ...
    並び.forEach(function (値) {
      ...
      var 割合 = Math.round(s.件数 / 全体 * 1000) / 10;
      Logger.log('  ' + 値 + ' … ' + s.件数 + '件（' + 割合 + '%）　（' + 内 + '）');
    });
    Logger.log('  ―― 合わせて ' + 全体 + '行・' + 並び.length + '種 ――');
    Logger.log('');
