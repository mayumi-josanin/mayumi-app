...
var 掲載書出_カレンダーの列 = {
  ...
  // **2026-09-05 追加。**書き出しに無かったため、サーバー側のカレンダーは
  // カテゴリが全部空だった（本番は70件に入っている）。
  // GAS は `ensureNamedColumn_(sheet, 'カテゴリ', 140)` で拾っている。
  category: ['カテゴリ'],
  published: ['公開設定'],
...
var 掲載書出_商品の列 = {
  ...
  // 5日後には22列になっていた。GAS はこの列だけを見て売切を判定している。
  // 無いまま出すと、売切の商品が「在庫あり」としてお客様に出る。
  sold_out: ['売切状態']
};
