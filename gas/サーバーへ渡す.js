...
// **2026-09-18、やり直しを強くした。**17日の昼から18日の朝まで、毎回のように
// 失敗が残っていた（30件すべてが2日ぶん）。読みは前回の答えでしのげていたが、
// **書き（processScheduledPushQueue）は5回落ちていた**。
// 500ミリ秒あけて2回では足りない。名前引きの失敗はしばらく覚えられる（負の記憶）ので、
// **間をだんだん広げて4回**まで粘る。名前引きは要求が届く前に失敗するので、
// やり直しても二重には書かれない。
var 渡す_やり直し回数 = 4;
var 渡す_やり直しの間ミリ秒 = [700, 2500, 6000];
...
function 渡す_取りに行く_(url, options) {
  ...
  for (var i = 0; i < 渡す_やり直し回数; i++) {
    try {
      ...
      if (!/DNS|アドレス/i.test(String(e))) throw e;
      if (i < 渡す_やり直し回数 - 1) Utilities.sleep(渡す_やり直しの間ミリ秒[i] || 6000);
    }
  }
  throw 最後;
}
