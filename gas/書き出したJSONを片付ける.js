...
var 片付けJSON_対象の形 =
  /^(会員データ|測定履歴|掲載物|売上|仕入|操作履歴|控えの記録|回数券分析|設定|ビジリス回答|ビジリスアンケート|ビジリス顧客|ビジリス設定)_\d{8}-\d{4}\.json$/;

// 中に個人情報が入るもの。片付けの下見で目立たせる。
//   会員データ … パスコードが平文
//   操作履歴   … 詳細JSONに、会員の編集内容（お名前・電話番号など）が入りうる
var 片付けJSON_要注意 = {
  '会員データ': 'パスコードが平文で入っています',
  '操作履歴': '詳細に会員のお名前や電話番号が入っている場合があります',
  '回数券分析': 'お客様のお名前・お体の分析結果・写真の場所が入っています',
  'ビジリス回答': 'お客様のお名前・回答・写真の場所が入っています',
  'ビジリス顧客': 'お客様のお名前・パスコードのハッシュ・メモが入っています'
};
...
function 片付けJSON_集める_() {
  ...
  while (it.hasNext()) {
    ...
    一覧.push({
      ...
      大きさ: f.getSize(),
      日: f.getDateCreated(),
      // 個人情報が入るものは、ひと目で分かるようにする。
      要注意: (function (名) {
        var 見つけた = '';
        Object.keys(片付けJSON_要注意).forEach(function (頭) {
          if (!見つけた && 名.indexOf(頭) === 0) 見つけた = 片付けJSON_要注意[頭];
        });
        return 見つけた;
      })(f.getName())
    });
